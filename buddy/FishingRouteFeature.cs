...
namespace HeartopiaMod
{
    ...
    public static class FishingRouteFeature
    {
        ...
        public static float DrawSection(HeartopiaComplete host, float startY)
        {
            float num = startY;
            ...
            if (host.UI_DrawPrimaryActionButton(new Rect(20f, num, 260f, 35f), actionLabel))
            {
                ...
                {
                    Stop(host);
                    host.UI_AddMenuNotification(host.UI_Localize("Fishing Locations stopped"), new Color(1f, 0.55f, 0.55f));
                }
                else
                {
                    Start(host);
                    host.UI_AddMenuNotification(host.UI_Localize("Fishing Locations started"), new Color(0.45f, 1f, 0.55f));
                }
            }
            num += 42f;
            ...
            if (active)
            {
                GUI.Label(new Rect(20f, num, 360f, 20f),
                    host.UI_LocalizeFormat("Location: {0}/{1} ({2})", currentIndex + 1, TotalSpotCount, GetSpotName(currentIndex)), small);
                num += 24f;
                GUI.Label(new Rect(20f, num, 360f, 20f), host.UI_LocalizeFormat("Route: {0}", host.UI_Localize(lastStatus)), small);
                num += 24f;
                if (pausedForRepair)
                {
                    GUIStyle warn = new GUIStyle(small);
                    warn.normal.textColor = new Color(1f, 0.7f, 0.45f);
                    GUI.Label(new Rect(20f, num, 360f, 20f), host.UI_Localize("Teleport paused until repair finishes"), warn);
                    num += 24f;
                }
            }
            ...
            if (customSpots.Count > 0)
            {
                GUI.Label(new Rect(20f, num, 320f, 20f), host.UI_LocalizeFormat("Custom spots: {0}", customSpots.Count), small);
                num += 24f;
                int removeIndex = -1;
                for (int i = 0; i < customSpots.Count; i++)
                {
                    HeartopiaComplete.CustomTeleportEntry entry = customSpots[i];
                    if (entry == null) continue;
                    Vector3 p = entry.position;
                    GUI.Label(new Rect(20f, num, 280f, 20f), $"{entry.name} ({p.x:F0}, {p.y:F0}, {p.z:F0})", small);
                    if (GUI.Button(new Rect(300f, num - 2f, 26f, 24f), "X"))
                    {
                        removeIndex = i;
                    }
                    num += 26f;
