...
| `FindAuraMonoClassByFullName("XD.GameGerm.Ecs.Boost.Extensions.EcsEntityExtensions")` | **Same first-image-only bug, second instance** (hit 2026-08-12 by the animal-roster trough lookup). `XD.GameGerm.Ecs*` has no prefix rule, so it lands in the default list → probes **XDTGameUI** only, while the class lives in **EcsClient** → silent miss, surfaced as "missing method(s): EcsEntityExtensions.GetNetId". Fix: `FindAuraMonoClassInImages(ns, name, ["EcsClient", "EcsClient.dll", …])`. **Rule of thumb: any namespace not in the `XDT*` / `EcsClient` / `ClientSystem` / `Sazabi` prefix table above needs an explicit image list.** ⚠ Also note `EcsEntityExtensions` exists TWICE — `XD.GameGerm.Ecs` (`IsAlive`) and `XD.GameGerm.Ecs.Boost.Extensions` (`GetNetId`); resolving the wrong one yields a valid class pointer whose method lookup then fails. |
