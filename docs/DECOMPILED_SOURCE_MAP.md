...
| Game LOD (world detail / draw distance) | Managers._serviceDic → IRenderingSystem.LoaderManager.SetParam + AreaPriorityManager.UpdateGlobalLodBias, IConfigManager.RenderLoadConfig getters (revert), LayerDistanceCulling.Instance, BrgManager.ForeceLOD0, RenderingSettings.Instance.GetCurrentURPAsset().shadowDistance (all EngineWrapper), SignificanceManager (DataModule).Enable, Entities.GetComponents&lt;LevelEntityComponent&gt; + SetForceNineCell/_nineCellRange (NineCell), TreeLoad.SetAllDynamicInstanceBlockEnable + HomeLoad.Inst (InstanceBlock rebake), PlayerPrefs "PC_LODBIAS", PhotoFrameComponent.PhotoLodDis static float[] element writes via mono_array_addr_with_size (the blank-UGC-photo fix — the game hardcodes a 100 m photo-REQUEST radius that vanilla's 24-30 m furniture spawn radius always stays inside; extending draw distance spawns frames past it so they never request → white); reference: EntityResLoad_NineCell, ObserverPanel (game's own debug blueprint) | GameLodFeature.cs (+ HeartopiaComplete.UguiGameLodContent.cs) | A |
