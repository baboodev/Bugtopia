...
- **Non-Latin glyphs** are not in LiberationSans, so a covering font asset is hunted (bundle-sweep cache → loaded assets → `TMP_Settings` → live scene text → the game's `fonts_*.ab` bundles off disk) and wired into its `fallbackFontAssetTable`; legacy `UnityEngine.UI.Text` is the last resort. Two things are tracked **per script** (`Han` / `Hangul` / `Kana` / `Thai`), never as one "CJK" flag:
  ...
  - **What is covered** — each script is hunted with its **own** probe glyph (`中` / `한` / `あ` / `ก`). A single `中` probe wires the Chinese and Japanese faces and rejects `Jua-Regular SDF`, the only Korean face in the bundles, because a Korean font has no Hanja — a chain with no Hangul in it renders Hangul as boxes exactly like no chain at all.
  The bundle sweep runs once per session and harvests every font it meets (`AssetBundle.LoadFromFile` throws for an already-loaded bundle, so a later script cannot re-open them). Only font assets the mod loaded itself may have their material flattened — a hit on a game-owned asset is wired in untouched. If a script the kit has actually been handed stays uncovered across several hunts, the whole kit switches to legacy Text, and switches back if the font turns up later.
