...
- **Session-only** and not saved to config: a pinned special-cooker type surviving a relog would make the next homeland capture 0 stoves for no visible reason. **Reset Capture** forgets it. Hidden in **Mini Game Only** (assist is cooker-type agnostic there) and disabled while Mass Cook is running (a mid-run retarget would abandon cooking stoves).
