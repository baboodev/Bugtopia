...
|---|---|---|---|
| Позиция NPC (в т.ч. НЕзагруженного) | AuraMono static `MapSpotProtocolManager.TryGetMapSpotPosition(SpotEnum.Npc=2, npcId, out Vector3)` | `Teleport.cs` → **`TryGetLiveNpcPositionByIdMono(int npcId, out Vector3)`** (~line 1050) | Работает для всего, что рендерит карта (server-synced map-spot сущность, движется вместе с NPC). Требует, чтобы NPC вообще был на карте текущей сцены |
| Позиция NPC (загруженного, Unity-скан) | `Object.FindObjectsOfType(Il2CppType.Of<NpcComponent>)` + чтение `position`/`entity.position`/`transform` | `Teleport.cs` → `PopulateLiveNpcEntriesFromUnityObjects` + `TryGetNpcTeleportPosition` | Только реально заспавненные Unity-объекты |
